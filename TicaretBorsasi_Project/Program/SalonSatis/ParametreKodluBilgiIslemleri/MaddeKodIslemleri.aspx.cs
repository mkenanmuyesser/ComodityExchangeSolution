    ...
    public partial class MaddeKodIslemleri : Page
    {
        ...
        protected void ButtonKaydetGuncelle_Click(object sender, EventArgs e)
        {
            using (var entity = new DBEntities())
            {
                ...
                if (Key == 0)
                {
                    data = new TT_MADDE_KOD1();
                }
                else
                {
                    data = entity.TT_MADDE_KOD1.SingleOrDefault(p => p.MaddeKodKey == Key);
                    if (data == null)
                    {
        ...
        protected void GridViewMaddeKodu_RowDeleting(object sender, ASPxDataDeletingEventArgs e)
        {
            using (var entity = new DBEntities())
            {
                ...
                TT_MADDE_KOD1 deleteddata = entity.TT_MADDE_KOD1.Single(p => p.MaddeKodKey == deletedkey);
                entity.TT_MADDE_KOD1.Remove(deleteddata);
        ...
        private void DataLoad()
        {
            using (var entity = new DBEntities())
            {
                ...
                GridViewMaddeKodu.DataSource = entity.TT_MADDE_KOD1.AsNoTracking().OrderBy(p => p.Kod).ToList();
                GridViewMaddeKodu.DataBind();

                if (Key != 0 && !IsPostBack)
                {
                    TT_MADDE_KOD1 data = entity.TT_MADDE_KOD1.AsNoTracking().Single(p => p.MaddeKodKey == Key);

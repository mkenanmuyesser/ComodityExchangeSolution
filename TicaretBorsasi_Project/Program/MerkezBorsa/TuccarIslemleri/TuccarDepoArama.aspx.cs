...
namespace TicaretBorsasi_Project.Program.MerkezBorsa.TuccarIslemleri
{
    public partial class TuccarDepoArama : Page
    {
        ...
        protected void ButtonRapor_Click(object sender, EventArgs e)
        {
            ...
            GridViewDepo.Columns["Birim1Column"].Visible = true;
            GridViewDepo.Columns["Birim2Column"].Visible = true;
            GridViewDepo.Columns["Bilgiler"].Visible = false;
            ...
            GridViewDepo.Columns["Birim1Column"].Visible = false;
            GridViewDepo.Columns["Birim2Column"].Visible = false;
            GridViewDepo.Columns["Bilgiler"].Visible = true;
        }
        ...
        private void DataLoad()
        {
            using (var entity = new DBEntities())
            {
                ...
                ComboBoxMaddeKodu.DataSource = entity.TT_MADDE_KOD.AsNoTracking().ToList();
        ...
        private void Ara()
        {
            using (var entity = new DBEntities())
            {
                ...
                int pMaddeKodu = ComboBoxMaddeKodu.SelectedIndex == -1
                                     ? 0
                                     : Convert.ToInt32(ComboBoxMaddeKodu.SelectedItem.Value);
                ...
                if (pMaddeKodu != 0)
                {
                    sonuc = sonuc.Where(p => p.MaddeKodKey == pMaddeKodu);
                }
                ...
                foreach (GridViewDataColumn item in GridViewDepo.GetGroupedColumns())
                {
                    GridViewDepo.UnGroup(item);
                }

                switch (ComboBoxGrupla.SelectedItem.Value.ToString())
                {
                    default:
                    case "1":
                        GridViewDepo.GroupBy(GridViewDepo.Columns["SicilNo"]);
                        break;
                    case "2":
                        GridViewDepo.GroupBy(GridViewDepo.Columns["Unvan"]);
                        break;
                    case "3":
                        GridViewDepo.GroupBy(GridViewDepo.Columns["MaddeKoduAdi"]);
                        break;
                }

...
{
    public partial class BeyannameTuccarTescilTakipDetay : Page
    {
        #region Properties
        ...
        protected void ButtonRapor_Click(object sender, EventArgs e)
        {
            ...
            {
                case 0:
                    GridViewExporterTuccarTescilTakip.WriteXlsxToResponse("Tüccar Tescil Takip Detay");
                    break;
                case 1:
                    GridViewExporterTuccarTescilTakip.WritePdfToResponse("Tüccar Tescil Takip Detay");
                    break;
                default:
                    break;
            }
        ...
        {
            LabelBaslik.Text = "TÜCCAR TESCİL TAKİP DETAY";
        ...
        private void Ara()
        {
            using (var entity = new DBEntities())
            {
                ...
                int? pBeyannameTipi = ComboBoxBeyannameTipi.SelectedIndex == 0 ? null : (int?)Convert.ToInt32(ComboBoxBeyannameTipi.SelectedItem.Value);
                ...
                string pBaslangicIlIlce = SpinEditBaslangicIlIlce.Text.Trim();
                string pBitisIlIlce = SpinEditBitisSubeKodu.Text.Trim();
                ...
                var sonuc = entity.BEYANs.Include("TUCCAR_SICIL")
                            ...
                            .Include("TUCCAR_SICIL.TT_IL_ILCE")
                            .AsNoTracking()
                            .ToList()
                            .Select(p => new
                            {
                                ...
                                IlKodu=p.TUCCAR_SICIL.TT_IL_ILCE.Kod,
                                MaddeAdi = p.TT_MADDE_KOD == null ? null : p.TT_MADDE_KOD.Adi,
                                ...
                                KarsiTuccar = p.TUCCAR_SICIL1 == null ? null : p.TUCCAR_SICIL1.Unvan,
                ...
                if (pBeyannameTipi != null)
                {
                    sonuc = sonuc.Where(p => p.BeyanTipKey == pBeyannameTipi);
                }
                ...
                if (!string.IsNullOrEmpty(pBaslangicIlIlce) || !string.IsNullOrEmpty(pBitisIlIlce))
                {
                    if (!string.IsNullOrEmpty(pBaslangicIlIlce) && string.IsNullOrEmpty(pBitisIlIlce))
                    {
                        int baslangic;
                        if (!int.TryParse(pBaslangicIlIlce, out baslangic))
                        {
                            PageHelper.MessageBox(this, "İl kodu girişi hatalıdır!");
                            return;
                        }

                        sonuc = sonuc.Where(p => p.IlKodu == pBaslangicIlIlce);
                    }
                    else
                    {
                        int baslangic;
                        int bitis;
                        if (int.TryParse(pBaslangicIlIlce, out baslangic) && int.TryParse(pBitisIlIlce, out bitis))
                        {
                            sonuc =
                                sonuc.Where(
                                    p =>
                                    Convert.ToInt32(p.IlKodu) >= baslangic && Convert.ToInt32(p.IlKodu) <= bitis);
                        }
                        else
                        {
                            PageHelper.MessageBox(this, "İl kodu girişi hatalıdır!");
